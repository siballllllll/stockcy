...
def _position_size(cash_krw: float, price: float, market: str,
    ...
    """[포지션 사이징 v3.136.2] 검증된 배수 근거가 없어 균등 배분으로 되돌림.
    (bb_pctb/mom_5 인자는 호출부 호환을 위해 남겨두지만 현재 사용하지 않는다.)
    ...
    대체 근거로 '눌림목 깊이'를 쓰려 했으나(같은 날 시도), 재검증에서 무효로 판명돼 철회했다.
    깊이-수익 단조 관계는 표본을 소스별로 가르자 `scenario`에서만 나타났다:
      scenario(197건) %b 0.00-0.05 71.0% → 0.17-0.25 47.5%  (깔끔한 단조 감소)
      pattern (62건)  %b 0.00-0.05 31.2% · 0.05-0.10 27.3% · 0.10-0.17 38.9% · 0.17-0.25 35.3%
                      (단조성 없음, 오히려 얕은 쪽이 나음. 5일 모멘텀도 중간 구간이 최고)
    그런데 scenario는 같은 날 예측력 0으로 판정해 ML 학습에서 제외한 소스다(EXCLUDED_SOURCES).
    한쪽에선 노이즈라 버리고 다른 쪽에선 사이징 근거로 쓰는 것은 일관성이 없으므로 채택하지 않는다.

    결론: 검증된 배수 근거가 현재 없다. 확신도(상수)도 ML(AUC 0.52)도 깊이도 근거가 못 되므로,
    근거 없는 차등 대신 **균등 배분**(현금의 12%)으로 둔다. 검증되지 않은 규칙으로 금액을
    차등하는 것보다, 아무 가정 없이 똑같이 나누는 편이 낫다.
    표본이 더 쌓이면(pattern 기준 200건+) 깊이·모멘텀 관계를 다시 재고 그때 도입을 판단할 것.

    반환 (qty, budget_krw). qty 0 = 예산 대비 주가가 너무 높아 진입 스킵."""
    budget = min(cash_krw * 0.12, cash_krw * 0.25)
