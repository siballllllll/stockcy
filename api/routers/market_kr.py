...
def kr_stock_price(code: str, fundamental: bool = Query(False, description="PER/PBR 등 펀더멘털 보강(네이버 스크래핑) — 검색 상세에서만 true")):
    ...
    result = get_kr_stock_price(code, with_fundamental=fundamental)   # KIS=실시간 정확. FDR 보강은 stale라 제거.
...
@router.get("/realtime-bulk")
def kr_realtime_bulk(codes: str = Query(..., description="콤마로 구분된 종목코드 — 정규장 실시간 시세(네이버)")):
    """국내 종목 정규장 실시간 시세 일괄 조회 (네이버). FDR(지연)·KIS(보합) 대신 실시간 표시용.
    화면용(즐겨찾기·보유·티커바, 수십 종목)에만 사용 — 스크리너 등 대량(50+)은 기존 bulk(FDR) 유지."""
    from data_kr import get_kr_realtime_price
    import concurrent.futures
    code_list = [c.strip().zfill(6) for c in codes.split(",") if c.strip()][:60]
    result: dict = {}
    if not code_list:
        return result
    with concurrent.futures.ThreadPoolExecutor(max_workers=10) as ex:
        futs = {ex.submit(get_kr_realtime_price, c): c for c in code_list}
