...
def ml_decisions_enabled() -> bool:
    """[v3.136.0] ML 예측을 '실제 매매 결정'에 반영할지 여부. 기본 off.

    2026-09-03 사후검증에서 사전에 정해둔 복귀 게이트(검증 100건+ AND 실전 AUC 0.6+)를
    명확히 미달했다:
      d3 137건 AUC 0.447(동전던지기 이하) · d7 117건 AUC 0.524 · d20 67건 AUC 0.530
      (학습 시점 CV AUC는 0.658이었음 — 과적합이 실측으로 확인된 것)
    예측 분포도 붕괴해 있다. d20은 예측값이 8~54%인데 실제 상승률은 65.7%로, 거의 모든
    종목에 '낮음'을 찍어 변별을 못 한다(섀도우 B가 실현 4건에 그친 것도 같은 원인 —
    ml7>=55 조건이 거의 발화하지 않았다).
    근본 원인은 학습 데이터: 748건 중 500건(67%)이 'scenario' 소스인데, 시나리오 종목은
    같은 날 예측력 0으로 측정됐다(수혜 31.0% vs 피해 34.9%, 표본 5,108건). 신호 없는
    라벨이 학습셋의 2/3를 차지하니 재학습만으로는 해결되지 않는다.

    복귀 절차: scenario 소스를 학습에서 제외하고 재학습 → 실전 AUC 재측정 → 0.6 이상이면
    ML_DECISIONS=1로 복귀. 그때까지 예측값은 계산·기록만 하고 판단에는 쓰지 않는다.
    섀도우 B(ML 순종)는 복귀 가능 여부를 재는 대조군이므로 계속 예측값을 받는다.
    """
    return os.getenv("ML_DECISIONS", "0") == "1"
...
def predict_win_proba(features: dict, horizon: str = "d7"):
    ...
    주의: 이 함수 자체는 항상 예측을 반환한다(기록·대조군용). 실제 매매 판단에 쓰기 전에
    호출부가 ml_decisions_enabled()를 확인할 것."""
    try:
