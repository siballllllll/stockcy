...
export const api = {
  ...
  ai: {
    ...
    peerCompare: (ticker: string, market: string = "국내") =>
      req(`/api/ai/peer-compare?ticker=${encodeURIComponent(ticker)}&market=${encodeURIComponent(market)}`),
  ...
  portfolio: {
    ...
    orderbook: (symbol: string)               => req<{ asks: Array<{ price: number; volume: number }>; bids: Array<{ price: number; volume: number }>; currency: string | null }>(`/api/stocks/orderbook?symbol=${symbol}`),
    ...
    priceLimits: (symbol: string)             => req<{ upper: number | null; lower: number | null; currency: string | null }>(`/api/stocks/price-limits?symbol=${symbol}`),
    stockMaster: (symbols: string[])          => req<Record<string, { name: string; english_name: string; market: string; security_type: string; status: string; list_date: string | null; delist_date: string | null; shares: string | null }>>(`/api/stocks/master?symbols=${symbols.join(",")}`),
    marketCalendar: (market = "KR")           => req<{ date: string | null; is_open: boolean; next_business_day: string | null; prev_business_day: string | null }>(`/api/market/calendar?market=${market}`),
