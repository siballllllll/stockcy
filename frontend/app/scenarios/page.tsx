...
export default function ScenariosPage() {
  ...
  const [issues, setIssues]       = useState<Issue[]>(() => {
    ...
    try { return JSON.parse(localStorage.getItem(SC_DATA_KEY) ?? "[]"); } catch { return []; }
  });
  const [lastUpdated, setLastUpdated] = useState<string>(() => {
    if (typeof window === "undefined") return "";
    return localStorage.getItem(SC_TS_KEY) ?? "";
  });
  ...
  useEffect(() => {
    if (typeof window !== "undefined" && !localStorage.getItem(SC_DATA_KEY)) {
      setFetchTrigger(0);
    }
  }, []);
