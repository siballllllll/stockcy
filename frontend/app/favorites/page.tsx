...
function FavRow({ fav, price, overtime, onRemove, onAnalyze, onSaveMemo, gapBulkMap, issues }: {
  ...
  // 열릴 때 화면 폭을 보고, 오른쪽으로 넘치는 만큼만 왼쪽으로 당긴다(무조건 우측정렬 X)
  useEffect(() => {
    ...
    const pad = 12;
    let off = 0;
    const rightEdge = rect.left + popW;
    if (rightEdge > window.innerWidth - pad) off = -(rightEdge - (window.innerWidth - pad));
    if (rect.left + off < pad) off = pad - rect.left;   // 왼쪽도 벗어나지 않게 클램프
    setMemoOffset(off);
  }, [memoOpen]);
    ...
    <div style={{
      ...
      <div style={{ display: "flex", gap: "5px", marginTop: "auto" }}>
        ...
        <div ref={memoRef} style={{ flex: 1, display: "flex", position: "relative" }}>
          ...
          {memoOpen && (
            <div style={{
              position: "absolute", top: "calc(100% + 6px)", left: memoOffset, zIndex: 50,
...
function HoldingsRiskBanner() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`${BASE_URL}/api/ai/holdings-risk`);
        if (res.ok) { const d = await res.json(); if (alive) { setFlagged(d.flagged ?? []); setLoaded(true); } }
      } catch { /* 무시 */ }
    })();
    return () => { alive = false; };
  }, []);
...
function PortfolioTab({ gapBulkMap }: { gapBulkMap: Record<string, any> }) {
  ...
  // KR 현재가 — 네이버 실시간(realtimeBulk). FDR은 장중 지연, KIS는 장초 보합이라 둘 다 부적합.
  const { data: krPrices } = useSWR(
    krTickers.length > 0 ? `kr-port-rt-${krTickers.join(",")}` : null,
    async () => {
      const rt = await api.kr.realtimeBulk(krTickers as string[]);
      const map: Record<string, number> = {};
      for (const code of krTickers) {
        const padded = String(code).trim().padStart(6, "0");
        const b = (rt as any)[padded] ?? (rt as any)[code];
...
export default function FavoritesPage() {
  ...
  // KR 시세는 네이버 실시간(realtimeBulk)으로 — FDR(StockListing)은 장중 지연(어제값), KIS는 장초 보합.
  // (w52는 없어 생략 → 바닥권/고점권 배지만 빠지고 나머지는 등락률로 정상 동작)
  const { data: krPriceMap } = useSWR(
    krTickers.length > 0 ? `kr-fav-rt-${krTickers.join(",")}` : null,
    async () => {
      const rt = await api.kr.realtimeBulk(krTickers as string[]);
      const map: Record<string, PriceEntry> = {};
      for (const code of krTickers) {
        const b = (rt as any)[code] ?? (rt as any)[String(code).padStart(6, "0")];
  ...
  const pollGapBulkFav = async (tickers: string[]) => {
    ...
    try {
      for (;;) {
        let job: any = null;
        try { job = await (await fetch(`${BASE_URL}/api/ai/overnight-gap-bulk/status`)).json(); } catch {}
        try {
  ...
  const handleGapBulkScan = async () => {
    ...
    const allTickers = Array.from(new Set([
      ...
      ...portfolioRes.map(p => String(p.ticker ?? "").toUpperCase().trim())
    ])).filter(Boolean);
    if (allTickers.length === 0) return;

    const names: Record<string, string> = {};
    (favs ?? []).forEach(f => { const t = f["티커"]?.toUpperCase().trim(); if (t) names[t] = f["종목명"] || t; });
    portfolioRes.forEach(p => { const t = String(p.ticker ?? "").toUpperCase().trim(); if (t && !names[t]) names[t] = p.name || t; });
