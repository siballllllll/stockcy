        ...
        def _ingest(scn):
            for iss in (scn or {}).get("issues", []) or []:
                ...
                if t and t not in main_map:   # 최신 우선(이미 있으면 유지)
                    econ_list = []
                    for a in (iss.get("scenarios") or [])[:2]:   # A·B 시나리오
                        txt = (a or {}).get("economic_analysis", "") or ""
                        if txt:
                            econ_list.append({
                                "label": a.get("label", "?"),
                                "direction": a.get("market_direction", ""),
        ...
        for r in rows:
            ...
            if len(out[d["ticker"]]) < 3:   # 종목당 최대 3개 이슈
                out[d["ticker"]].append({
                    ...
                    "economics": info.get("economics") or [],
                })
        return out
    except Exception as e:
...
def load_confluence_picks(days: int = 5, min_engines: int = 2) -> list:
    ...
    try:
        ...
        for r in cur.fetchall():
            ...
            _hsh = str(r.get("hsh_label") or "").strip()
    ...
    for k, a in agg.items():
        if len(a["engines"]) >= min_engines:
            ...
            # 교차검증 성립일: 엔진별 '최초 포착일'을 정렬해 min_engines번째 = 두 번째(이상) 엔진이 합류해
            # 교차검증이 성립한 날. (first_date는 한 엔진의 최초 포착, confluence_date는 겹친 시점)
        ...
        for p in picks:
            cursor.execute(
                """INSERT INTO screener_picks
                   (picked_date, ticker, name, match_score, signal, rsi, vol_ratio, pos_52w, ma_aligned, price, hsh_label)
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                (today, _pad_kr_ticker(p.get("code", "")), p.get("name", ""),
                 p.get("match_score"), p.get("signal"), p.get("rsi"),
                 p.get("vol_ratio"), p.get("pos_52w"),
                 1 if p.get("ma_aligned") else 0, p.get("current_price"), p.get("hsh_label") or "")
            )
        conn.commit()
        conn.close()
    except Exception as e:
