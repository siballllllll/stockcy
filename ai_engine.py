...
'지금 장이 왜 이렇게 극단적으로(또는 한쪽으로 쏠려) 움직이는가'를 날카롭게 해부하는 칼럼을 작성하세요.

[톤·깊이]
- 날카롭고 직설적으로. 시장의 구조적 모순과 위험을 에둘러 말하지 말고 명확하게 지적하세요. (단, 근거 없는 공포 조장이나 단정은 금지 — 반드시 메커니즘·데이터로 뒷받침)
- 충분히 깊고 길게: 8~12개 단락. 각 논점마다 '왜 그런가'의 메커니즘과 구체적 수치·사례·종목을 들어 설명하세요. 표면적 요약 금지.

[다뤄야 할 논점]
- 구조적 요인: 패시브·인덱스 ETF 자금 집중, 알고리즘 동조매매, 군집행동이 어떻게 변동성을 '증폭'시키는지 메커니즘으로.
- 수급의 비대칭: 위 실데이터(외국인·기관 순매수/순매도 종목)와 검색으로 확인한 수치를 근거로, 자금이 어디로 쏠리고 어디서 빠지는지 구체적으로.
- 심리적 요인: 센티먼트 쏠림, 공포·탐욕, '되는 놈만 되는' 양극화가 개인을 어떻게 함정에 빠뜨리는지.
- 펀더멘털 vs 심리: 지금 움직임이 실적 때문인지 심리 때문인지 냉정하게 구분.
- 리스크 시나리오: 이 쏠림이 풀릴 때(되돌림) 무엇이 터질 수 있는지 1~2가지.

[마무리]
- '이런 장에서 개인투자자가 반드시 지켜야 할 냉정한 원칙' 3~4줄. 구체적 행동 지침으로.

형식: 첫 줄은 제목(마크다운 기호 없이 한 줄, 도발적이되 과하지 않게), 그 다음 줄부터 본문(마크다운 문단, 핵심은 **굵게**, 적당한 이모지)."""
    try:
        response = _call_gemini(prompt, use_search=True, temperature=0.65,
