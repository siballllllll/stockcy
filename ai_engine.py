...
[핵심 지시 — 낙관 편향 절대 금지]
비추천·관망 의견이어도 "장기적으로는 오를 수 있다"식으로 희석하지 마세요.
① 현재 이 종목의 핵심 리스크·부정 이슈를 먼저 파악하고,
② 그 이슈를 근거로 근 시일 내 하락 가능성을 구체적 수치로 제시한 뒤,
③ 이슈 해소 또는 반등 시의 중기 시나리오를 조건부로 제시하세요.
낙관적 면이 없으면 억지로 만들지 말고 솔직하게 하락 전망만 써도 됩니다.

구글 검색으로 최신 뉴스·실적·SEC 공시·옵션 플로우를 파악한 뒤 반드시 아래 JSON 형식으로만 응답하세요:
{{
  "rating": "단기 트레이딩 등급 (매우 강력 추천 / 추천 / 중간추천 / 비추천 / 매우 비추천)",

  "key_issues": "현재 이 종목의 핵심 리스크·부정 이슈 2~3가지 (마크다운 불릿, 실적 미스·가이던스 하향·섹터 역풍 등 구체적으로)",

  "short_term_downside_pct": "근 시일(1~4주) 하락 가능성 수치 (예: -8 ~ -12%). 상승 우세하면 양수로",
  "short_term_downside_price": "하락 시 예상 도달 가격대 또는 1차 지지선 (달러 단위)",
  "short_term_downside_reason": "이 가격대까지 내려올 수 있는 구체적 이유 (이슈·수급·기술적 요인, 2~3문장)",

  "buy_target": "매수 적정 구간 / 추격매수 금지선 (달러 단위)",
  "sell_target": "단기 목표가 (달러 단위)",
  "stop_loss": "손절가 (달러 단위)",

  "mid_term_upside_pct": "중기(1~3개월) 반등·상승 여력 수치 (예: +15 ~ +25%). 회복 불투명하면 하락폭으로",
  "mid_term_upside_price": "중기 기준 예상 가격대 (달러 단위)",
  "mid_term_upside_condition": "이 수준까지 오르기 위한 전제 조건 또는 catalyst (구체적인 이벤트·변화)",
  ...
  "long_term_target_pct": "중장기 기대 수익/손실률",
...
[핵심 지시 — 낙관 편향 절대 금지]
비추천·관망 의견임에도 "장기적으로는 상승 가능"식으로 희석하지 마세요.
반드시 ① 현재 존재하는 부정적 이슈/리스크를 먼저 파악하고,
② 그 이슈를 근거로 근 시일 내 하락 가능성을 구체적 수치로 제시한 뒤,
③ 하락 이후 또는 이슈 해소 시의 중기 반등 시나리오를 조건부로 제시하세요.
낙관적 면이 없으면 억지로 만들지 말고 솔직하게 하락 전망만 써도 됩니다.
...
구글 검색으로 최신 뉴스·실적·공시·섹터 이슈를 파악한 뒤 반드시 아래 JSON으로만 응답하세요.
{{
  "rating": "단기 트레이딩 등급 (매우 강력 추천 / 추천 / 중간추천 / 비추천 / 매우 비추천)",

  "key_issues": "현재 이 종목에 존재하는 핵심 리스크·부정 이슈 2~3가지 (마크다운 불릿, 실적 우려·외인 이탈·섹터 역풍 등 구체적으로)",

  "short_term_downside_pct": "근 시일(1~4주) 하락 가능성 수치 (예: -8 ~ -12%). 상승 가능성 높으면 양수로",
  "short_term_downside_price": "하락 시 예상 도달 가격대 또는 1차 지지선 (예: 65,000원 수준)",
  "short_term_downside_reason": "이 가격대까지 내려올 수 있는 구체적 이유 — 이슈, 수급, 기술적 요인 포함 (2~3문장)",
  ...
  "mid_term_upside_pct": "중기(1~3개월) 반등·상승 여력 수치 (예: +15 ~ +25%). 회복 불투명하면 하락폭으로",
  "mid_term_upside_price": "중기 기준 예상 가격대 (원 단위)",
  "mid_term_upside_condition": "이 수준까지 오르기 위한 전제 조건 또는 catalyst (어떤 이벤트·변화가 있어야 하는지 구체적으로)",

  "세력분석": "외국인/기관 수급 흐름의 의미를 2~3문장으로 분석",
  "analysis": "종합 단타 전략 (최신 뉴스, 차트 패턴, 진입 근거 등 마크다운 상세 작성)",
  "historical_pattern_analysis": "현재 주가 흐름·수급·섹터 모멘텀과 유사했던 과거 패턴(프랙탈) 1~2개, 당시 결과 비교 (마크다운)",

  "long_term_rating": "중장기 등급 (적극 매수 / 분할 매수 / 관망 / 비중 축소 / 전량 매도)",
  "long_term_period": "권장 투자 기간 (예: 3~6개월, 6개월 이상)",
  "long_term_target": "중장기 목표가 (원 단위, 3~6개월 관점)",
  "long_term_target_pct": "중장기 기대 수익/손실률",
  "long_term_analysis": "거시경제 사이클·펀더멘털 중장기 분석 (마크다운 상세)"
}}
