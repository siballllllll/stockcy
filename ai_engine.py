...
def generate_market_scenarios() -> dict:
    ...
    prompt = (
        ...
        "- 대장주(1개): 해당 테마에서 가장 직접 수혜받는 핵심 종목. 반드시 1개 포함.\n"
        "- 직접관련주(1~2개): 사업 구조상 직접 관련이 있는 국내 종목. 반드시 1개 이상 포함.\n"
        "- 간접테마주(1~3개): 약하게 관련되지만 과거 유사 이슈 때 시장 심리로 함께 급등한 종목. 역사적 패턴 근거 있으면 반드시 포함.\n"
        "- 총 theme_stocks는 대장주·직접관련주·간접테마주를 합쳐 최소 3개 이상 포함하세요. 억지로 연결하지 않되, 실제 반복된 패턴이 있는 종목은 빠짐없이 넣으세요.\n"
        "- theme_stocks는 국내(KOSPI/KOSDAQ) 종목만 포함하세요.\n"
        "- ⚠️ theme_stocks에는 rising_stocks·falling_stocks에 이미 포함된 종목을 절대 중복 포함하지 마세요. 테마 연동주는 상승/하락 예상 종목과 완전히 다른 종목으로만 구성해야 합니다.\n\n"
