...
def analyze_sector_rotation(market_type, raw_market_data):
    ...
    다음 섹터 예측, 추천주, 진입 타이밍까지 종합 분석합니다.
    """
    prompt = f"""
    ...
    [필수 포함 내용]
    1. 🚀 현재 시장 에너지 진단 및 근거
       - 현재 어떤 섹터가 시장을 주도하고 있으며, 그 상승의 동력이 무엇인지(수급, 뉴스, 매크로) 기술하세요.
    
    2. 🧭 다음 순환매 이동 경로 예측 (바통 터치)
       - 자금이 다음에 어느 섹터로 이동할지 과거 역사적 패턴과 현재 지표를 근거로 예측하세요.
       - 왜 '지금' 이 섹터로 자금이 옮겨갈 수밖에 없는지 논리적 근거를 명시하세요.

    3. 💎 TOP 3 추천 종목 및 실전 전략
       - 예측한 섹터에서 가장 유망한 종목을 섹터당 1~2개씩 선정하세요.
       - 각 종목에 대해:
         * 현재 주가 상태 진단 (저평가/돌파 중/과열 등)
         * 추천 진입 타이밍 (예: 현재가 즉시, OO원 눌림목 대기, OO선 돌파 확인 후)
         * 기대 수익률 및 목표 보유 기간

    4. ⚠️ 리스크 관리
       - 이 시나리오가 틀릴 수 있는 변수와 대응책을 제시하세요.

    형식: 전문가의 브리핑 느낌으로 정중하고 명확하게 작성하세요. 
    중요한 수치나 종목명은 **강조** 처리하고, 가독성 있게 마크다운 표나 리스트를 활용하세요.
    제목은 '🚀 [종합] {market_type} 시장 자금 흐름 & 차기 주도주 로드맵'으로 하세요.
    """
    try:
        # 실전 분석이므로 검색 기능을 활용해 최신 매크로 정보를 보강합니다.
        response = _call_gemini(prompt, use_search=True, temperature=0.7)
