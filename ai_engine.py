...
def _diversify_probabilities(res: dict) -> dict:
    """확률(probability_pct) 정규화 — A+B=100 보정과 라벨 일치만 수행한다.

    [v3.134.0] 원래 이 함수는 5의 배수(60·65·70…)가 들어오면 제목 해시로 ±1~3 흔들어
    62·73처럼 '정밀해 보이게' 만들었다. 그러나 그 흔들기는 정보량이 0이면서 정밀도만
    가장하는 것이라, 60%를 62%로 바꿔도 더 정확해지지 않고 오히려 근거 있는 수치처럼
    읽히게 만든다. 확률 보정(calibration) 측정을 시작하는 마당에 관측값을 인위적으로
    훼손할 이유가 없어 제거했다. 둥근 값이 나오면 둥근 값 그대로 기록한다.
    """
    def _to_int(v):
        try:
    ...
    for issue in res.get("issues", [res]):
        ...
        if a is None:
            continue
        a = max(5, min(95, a))             # 범위만 클램프 (값 자체는 모델이 낸 그대로 보존)
...
def generate_market_commentary() -> dict:
    ...
    prompt = f"""당신은 거시경제·환율·수급·정책을 두루 꿰는 한국 주식시장 칼럼니스트입니다. 반드시 모든 출력을 한국어(한글)로 작성하세요. 영어 문장으로 답변하면 안 됩니다 — 영문은 종목 티커·기업 고유명사에만 허용합니다. 한자(漢字)는 절대 금지.
오늘은 {_today}입니다. 지금 즉시 구글 실시간 검색으로 최근 1~2주 한국·미국 증시와 거시 환경을 폭넓게 파악하세요: 미국 고용·물가 지표와 연준 금리경로, 달러/원 환율과 엔화 동향, 유가·미국채 금리, 외국인·기관 수급, 트럼프 행정부 정책(관세·제조업 리쇼어링·금리 압박)과 미국 중간선거, 반도체 수출·실적 사이클, 주도 테마와 소외 업종.

[참고 실데이터]
{ctx}

'지금 시장을 움직이는 진짜 동인이 무엇인가'를 다각도로 해부하는 칼럼을 작성하세요.

[가장 중요한 원칙 — 매번 다른 글이 나와야 함]
- 매번 똑같은 '패시브 ETF·알고리즘 쏠림·군집행동' 프레임을 기본값으로 반복하지 마세요. 그것은 오늘 검색으로 확인한 실제 주된 동인일 때만 쓰세요.
- 오늘 검색으로 확인한 '오늘자 핵심 동인' 1가지를 메인 앵글로 잡고, 아래 [관점 메뉴]에서 오늘 가장 적합한 보조 관점 2~3개를 골라 엮으세요. 날마다 메인 앵글과 조합이 달라져야 합니다.
- 추상적 일반론 금지. 구체적 수치(환율 레벨, 경상수지 규모, 순매수 금액, 지표 수치, 종목명)를 반드시 인용하세요.

[관점 메뉴 — 오늘 가장 설명력 있는 것들을 골라 깊게 파세요]
- 환율 구조: 달러/원 레벨과 방향. 경상수지 흑자에도 원화가 약세인 자본이동 메커니즘(외국인 리밸런싱 매도, 국민연금·서학개미 해외투자 유출, 수출 대기업의 달러 보유, 대미 투자 약속, 엔저 동조 등)을 데이터로.
- 글로벌 매크로: 미국 고용·물가, 연준 금리경로(인상/인하/동결 베팅), 유가, 미국채 금리, 달러인덱스가 한국 증시에 미치는 경로.
- 정책·정치: 트럼프 행정부의 관세·제조업 리쇼어링·금리 압박, 미국 중간선거 일정과 그 이후 시나리오, 한국 정책 변수.
- 수급 구조: 위 실데이터(외국인·기관 순매수/순매도)와 검색 수치를 근거로 자금이 어디로 쏠리고 빠지는지. 패시브·국민연금·개인 해외투자 흐름.
- 펀더멘털: 반도체 수출·실적 사이클, 업종별 이익 모멘텀. 지금 움직임이 실적 때문인지 심리 때문인지 냉정히 구분.
- 심리·행동: 센티먼트 쏠림, 공포·탐욕, '되는 놈만 되는' 양극화가 왜 나타나고 어떻게 전개되는지의 흐름.
- 구조적 아이디어·전망: 환율·수급 문제에 대한 구조적 대응(예: 삼성전자·하이닉스의 미국 ADR 상장 가능성 등)이나 향후 분기 시나리오.

[톤·깊이 — 신문 경제칼럼처럼]
- 좋은 시장 칼럼니스트의 목소리로 쓰세요: 차분하고 통찰력 있게, 현상을 '이야기'로 풀어주는 어조. 독자가 '아, 그래서 이렇게 움직였구나' 하고 무릎을 치게.
- 비판이 목적이 아니라 '이해'가 목적입니다. 위험과 모순도 다루되, 경고·훈계·공포 조장 톤이 아니라 균형 잡힌 관찰자의 시선으로. 좋은 점과 우려스러운 점을 함께 짚으세요.
- 단정·과장 금지. 모든 주장은 메커니즘·데이터로 뒷받침하고, 불확실한 것은 '~할 가능성', '지켜볼 대목' 식으로 열어두세요.
- 충분히 깊고 길게: 7~11개 단락. 각 논점마다 '왜 그런가'의 메커니즘과 구체적 수치·사례·종목을 들어 설명하세요. 표면적 요약 금지.

[마무리 — 칼럼의 맺음말]
- 이 흐름이 앞으로 어떻게 전개될 수 있는지, 무엇을 지켜봐야 하는지(쏠림 해소, 정책 변화, 금리 방향 전환 등) 차분히 짚으며 닫으세요.
- 훈계조의 '반드시 지켜야 할 원칙' 대신, 이 장을 바라보는 투자자에게 건네는 사려 깊은 통찰 2~3줄로 여운 있게 마무리하세요.
...
def _generate_market_scenarios_legacy() -> dict:
    ...
    prompt = (
        ...
        "【종목 선정 규칙 — 개수보다 연결고리가 중요합니다】\n"
        "- ⛔ 칸을 채우지 마세요. 이 규칙이 다른 모든 종목 규칙보다 우선합니다.\n"
        "  falling_stocks는 **비워도 됩니다**. 이 이슈로 실제 실적·비용이 악화되는 종목이 뚜렷하지 않으면 빈 배열 []을 쓰세요.\n"
        "  '수혜 하나, 피해 하나' 식으로 균형을 맞추려고 억지 배정하는 것은 명백한 실패입니다.\n"
        "  실제 관측된 실패 사례: 중국 부동산 부양책 이슈에서 대우건설을 수혜, 현대건설을 피해로 나눠 배정한 적이 있습니다.\n"
        "  같은 정책의 같은 업종인데 한쪽만 피해로 몰아넣은 것으로, 이런 배정은 절대 하지 마세요.\n"
        "- rising_stocks는 2~3개면 충분합니다. 각 종목의 reason에는 **이 이슈가 그 회사의 매출·비용·수주·규제 중\n"
        "  무엇을 통해 영향을 주는지 구체적 경로**를 쓰세요. 경로를 한 문장으로 못 쓰겠으면 그 종목은 빼세요.\n"
        "  (나쁜 예: '반도체 대표주라 수혜' / 좋은 예: 'BOJ 엔화 강세 시 일본 경쟁사 대비 가격경쟁력 개선 → 수출 단가 방어')\n"
        "- ⛔ 서로 다른 이슈에 같은 대형주 묶음(삼성전자·현대차·LG화학 등)을 반복해서 넣지 마세요.\n"
        "  실제 관측된 실패 사례: '일본 BOJ 금리 정책'과 '미국 제조업 PMI'라는 완전히 다른 이슈에\n"
        "  삼성전자(수혜)·현대차(피해)·LG화학(테마)이라는 똑같은 구성이 나온 적이 있습니다.\n"
        "  파급 경로가 다르면 종목도 달라야 합니다. 이슈마다 그 이슈에 고유한 종목을 찾으세요.\n"
        ...
        "【추가 관련주 선정 규칙 — theme_stocks】\n"
        "theme_stocks는 rising_stocks(주로 대장주) 외에, 이 이슈로 수혜가 예상되는 '추가 종목' 섹션입니다.\n"
        "단타 급등주만 담지 말고 중장기 보유 관점의 종목도 함께 담아 균형을 맞추세요.\n"
        "핵심: 대형주(삼성전자·SK하이닉스·현대차 등 시총 10조↑)는 이미 rising_stocks에 있으므로 여기엔 절대 포함하지 마세요.\n"
        "✅ 구성 (총 2~4개, 국내 KOSPI/KOSDAQ만, 각 종목에 horizon 반드시 명시):\n"
        "   개수를 채우는 것보다 연결고리가 분명한 종목만 담는 것이 중요합니다. 2개만 확실하면 2개만 쓰세요.\n"
        "- 단타·스윙주 (1~2개, horizon='단타'): 이슈에 직접 연동되거나 과거 동일 이슈 때 시장 심리로 함께 급등한 이력이 있는 중소형주. 시총 1조 미만 코스닥 우선, 하루 5~15% 변동 가능. 역사적 패턴 근거 권장.\n"
        "- 중장기주 (1~2개, horizon='중장기'): 이 이슈의 구조적 수혜를 받을 중소형~중형 성장주·가치주. 단기 급등보다 실적 성장·펀더멘털 관점으로 선정. 시총 제한은 완화(중형 허용)하되 10조↑ 대형주는 제외.\n"
        ...
        "- 근거가 실제로 한쪽에 기울 때만 확률을 벌리세요. 근거가 팽팽하면 50/50이나 55/45라고 정직하게 쓰는 것이 맞습니다 — 억지로 벌린 숫자는 틀린 숫자입니다.\n"
        ...
        "- 확률은 실제 확신도만큼만 세밀하게 쓰세요. 70%면 70, 73%라고 볼 근거가 있으면 73 — 정밀해 보이려고 끝자리를 꾸미지 마세요(그 자릿수는 나중에 실제 적중률과 대조해 검증됩니다).\n"
        ...
        '          "label": "A",\n'
        '          "title": "시나리오 제목",\n'
        '          "probability_pct": 확률 정수(5~95, [확률 산정 규칙] 준수 — A+B=100),\n'
        ...
        "          ],\n"
        '          "theme_stocks": [\n'
        '            {"name": "종목명", "ticker": "KOSPI/KOSDAQ 6자리숫자코드", "horizon": "단타 또는 중장기", "type": "직접관련주 또는 간접테마주", "historical_pattern": "과거 유사 이슈 때 이 종목이 어떻게 움직였는지 (1문장, 중장기주는 생략 가능)", "reason": "이번에 수혜가 예상되는 이유 + 시총 규모 간략 언급", "signal": "매우 강력 추천/추천/중간추천/비추천/매우 비추천", "signal_reason": "현재 매수 관점 한 줄 요약", "expected_gain_pct": "기대 상승률 (% 기호 없이, 단타는 단기·중장기는 보유기간 기준, 예: 12.0)", "expected_loss_pct": "손절 기준율 (음수, 예: -4.0)"}\n'
        "          ],\n"
...
def _deep_dive_issue(stub: dict) -> dict:
    ...
    prompt = (
        ...
        "【종목 선정 규칙 — 개수보다 연결고리】\n"
        "- ⛔ 칸을 채우지 마세요. falling_stocks는 **비워도 됩니다**(빈 배열 []). 실제로 실적·비용이 악화되는\n"
        "  종목이 뚜렷하지 않은데 '수혜 하나 피해 하나'로 균형 맞추려 억지 배정하는 것은 실패입니다.\n"
        "  (실패 사례: 중국 부동산 부양책에서 대우건설=수혜, 현대건설=피해로 나눈 배정 — 같은 정책·같은 업종인데 임의로 갈랐음)\n"
        "- rising_stocks 2~3개. reason에는 이 이슈가 그 회사의 매출·비용·수주·규제 중 무엇을 통해 영향을 주는지\n"
        "  구체적 경로를 쓰세요. 경로를 한 문장으로 못 쓰면 그 종목은 빼세요.\n"
        "- ⛔ 서로 다른 이슈에 같은 대형주 묶음(삼성전자·현대차·LG화학 등)을 반복 배치 금지.\n"
        "  (실패 사례: 'BOJ 금리'와 '미국 제조업 PMI'에 삼성전자·현대차·LG화학이 똑같은 역할로 나왔음)\n"
...
def analyze_custom_issue(keyword: str) -> dict:
    ...
    prompt = (
        ...
        "【종목 선정 — 개수보다 연결고리】\n"
        "- ⛔ 칸 채우기 금지. falling_stocks는 비워도 됩니다(빈 배열 []). 실제로 악화되는 종목이 뚜렷하지 않은데\n"
        "  '수혜 하나 피해 하나'로 균형 맞추려 억지 배정하지 마세요.\n"
        "- rising_stocks 2~3개. reason에 이 이슈가 매출·비용·수주·규제 중 무엇을 통해 영향을 주는지 경로를 쓰세요.\n"
...
def analyze_autonomous_trading(ticker: str, name: str, current_price: float, market: str, position: str, avg_price: float,
    ...
    try:
        ...
        # 신규 매수일 때만 포지션 성격(horizon) 지정 — 강제 청산 가드가 차등 적용됨(v3.113.0)
        hz_field = (
            ',\n  "horizon": "BUY 결정 시에만: \'swing\'(수일 내 스윙, 손절 -3% 원칙) 또는 \'long\'(수주~수개월 중장기 — 차트 구조·기업가치·섹터 흐름이 장기 보유를 지지할 때만)"'
...
def _apply_volatility_anchor(res: dict, anchor: dict) -> dict:
    ...
    for field, band_key in _ANCHOR_FIELD_BANDS:
        ...
        if hit:
            ...
            res[field] = fixed
    # basis 라벨 교차검증 — 자기신고라 어긋난다. 실측: AI가 upside를 밴드 p90 그대로 적고도
    # "촉매반영: HBM 점유율 확대"라고 라벨링한 사례 확인. 수치가 밴드 값과 사실상 같으면
    # 그건 정의상 기저율이므로, 라벨을 코드가 바로잡는다(클램프와 같은 철학 — 지시가 아니라 강제).
    _obs = {"upside_scenario_pct": anchor.get("obs_4w"), "downside_scenario_pct": anchor.get("obs_4w")}
    for field, st in _obs.items():
        bkey = field.replace("_pct", "_basis")
        if not st or field not in res or bkey not in res:
            continue
        try:
            nums = re.findall(r'[-+]?\d+(?:\.\d+)?', str(res.get(field)))
            if not nums:
                continue
            v = abs(float(nums[0]))
        except Exception:
            continue
        band_vals = [st.get("typical"), st.get("p90"), st.get("p95")]
        if any(b and abs(v - float(b)) <= 0.15 for b in band_vals):
            if not str(res.get(bkey) or "").startswith("기저"):
                res[bkey] = "기저 (실측 변동폭 값과 동일 — AI가 촉매반영이라 적었으나 시스템이 교정)"
...
[손절 vs 보유 판단 원칙 — 매우 중요] 단순히 많이 떨어졌다는 이유만으로 손절하지 마세요. (1) 실적·성장성·산업 전망 등 펀더멘털과 미래 가치가 여전히 살아있으면 급락은 오히려 분할매수/보유 기회일 수 있습니다. (2) 반대로 미래 가치·논리가 훼손(실적 악화·구조적 악재·추세 붕괴)됐다면 손실이 작아도 손절이 맞습니다. → 손절 결정은 '하락폭'이 아니라 '미래 가치 훼손 여부'로 판단하고, stop_loss·reason에 그 근거를 명확히 적으세요.
...
[가격대 다각도 평가 — 매우 중요] '현재 수익률(평단 대비 손익 {sign}{pnl_pct:.1f}%)'은 '매수 타이밍'의 결과일 뿐, 지금 가격이 싼지 비싼지와는 별개입니다. 그 숫자(−15%든 −5%든)에 휘둘리지 말고 현재 '가격 자체'를 ①밸류에이션(PER/PBR·이익) ②52주 위치·주요 지지/저항 ③최신 이슈·업황 ④외국인·기관 수급 등 여러 각도로 평가해 '저평가/적정/고평가 구간'인지 판정하고 price_zone에 적으세요.

[일관성 원칙 — 매우 중요] 펀더멘털이 동일하다면 같은 종목은 '더 낮은 가격'이 추가매수에 '더 유리'합니다. 따라서 −15%(저점)에선 손절을 권하다가 −5%로 회복하자 추가매수를 권하는 식의 모순된 판단을 절대 하지 마세요. 추매가 타당한 종목이면 '더 쌌을 때(더 큰 하락)'를 오히려 더 적극적인 기회로 봐야 하고, 반대로 손절이 맞다면 회복했어도 추매가 아니라 비중축소가 맞습니다. 즉 손절/추매 방향은 '현재 손익률'이 아니라 '가치 대비 가격대 + 펀더멘털 훼손 여부'로 일관되게 결정하세요.
...
{{
  ...
  "price_zone": "현재 '가격대' 자체를 다각도(밸류·52주위치·지지/저항·이슈·수급)로 본 평가 — '저평가 구간/적정 구간/고평가 구간' 중 하나로 시작하고 근거 1~2문장. 평단 대비 손익률과 무관하게 '가격 자체' 기준으로 판단",
  ...
  "stop_loss": "★ 손절가 — 이 가격이 깨지면 손실 확정·청산하라는 구체적 가격(직전 지지선/주요 이탈선 기준). 현재가 대비 손절폭(%)도 함께. 손익비 관점에서 반드시 제시",
  ...
  "post_avg_exit": "평단을 낮춘 뒤 권장 매도 타이밍·목표가 — 새 평단 기준 본전/수익 구간을 어디서 어떻게 청산할지 (구체 조건)"
    ...
    try:
        ...
        # 검색 그라운딩은 가끔 JSON 아닌 응답(서두 텍스트·인용)을 줘서 파싱 실패 → 1회 재시도(할당량 초과는 즉시 중단).
        res = None
        _last = None
        for _attempt in range(2):
            try:
                ...
                _last = _pe
                if any(k in str(_pe) for k in ("QUOTA", "429", "RESOURCE_EXHAUSTED")):
                    raise
        if res is None:
...
{{
  ...
  "upside_scenario_basis": "이 수치의 출처를 둘 중 하나로 명시. '기저' = 특정 촉매·악재 없이 이 종목의 통상 변동폭을 그대로 쓴 경우(=위 실측 변동폭 값). '촉매반영: <무엇>' = 확인된 재료·악재 때문에 기저에서 벗어난 경우, 그 근거를 함께. ⚠️ 실측 변동폭 값을 그대로 적었다면 반드시 '기저'라고 쓰세요 — 이걸 촉매반영이라고 하면 거짓입니다.",
  ...
  "downside_scenario_basis": "이 수치의 출처를 둘 중 하나로 명시. '기저' = 특정 촉매·악재 없이 이 종목의 통상 변동폭을 그대로 쓴 경우(=위 실측 변동폭 값). '촉매반영: <무엇>' = 확인된 재료·악재 때문에 기저에서 벗어난 경우, 그 근거를 함께. ⚠️ 실측 변동폭 값을 그대로 적었다면 반드시 '기저'라고 쓰세요 — 이걸 촉매반영이라고 하면 거짓입니다.",
...
def discover_hot_day_trading_stock(context=""):
    ...
    """
    prompt = f"""
    당신은 가상의 주식 트레이딩 교육 전문가입니다.\n반드시 모든 출력을 한국어(한글)로 작성하세요. 영어 문장으로 답변하면 안 됩니다 — 영문은 종목 티커·기업 고유명사에만 허용합니다. 한자(漢字)는 절대 금지. 본 분석은 실제 투자 권유가 아니라 교육적 목적으로만 사용합니다.
...
def screen_by_my_pattern() -> dict:
    ...
    # 4. Gemini 최종 판단
    # 'win'(승리거래 백분위)이 None일 수 있다(승리표본 부족 등) → 빈 dict로 가드(크래시 방지).
    if dual_mode:
...
{{
  "holdings": [
    {{
      "ticker": "종목코드",
      "name": "종목명",
      "decision": "HOLD | TAKE_PROFIT | ROTATE | CUT_LOSS",
      "decision_label": "🟢 홀딩 | 🔴 차익실현 | 🔄 로테이션 | 🛑 손절 중 하나",
      "reason": "판단 근거 1~2문장 (RSI·52주위치·수급 종합)",
      "stop_loss": "CUT_LOSS인 경우 손절 기준가(직전 지지선/이탈선)와 이유. 그 외엔 빈 문자열",
...
- ★ 추세 붕괴(MA 정배열 깨짐·하락 지속)나 손실 확대로 상승 논리가 훼손됐으면 → CUT_LOSS(손절). stop_loss에 손절 기준가 제시. '버티면 오르겠지'식 방치 금지.
