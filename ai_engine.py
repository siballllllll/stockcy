...
def generate_market_scenarios() -> dict:
    ...
    prompt = (
        ...
        "주요 이슈 5~6개를 파악하세요. 반드시 비트코인·암호화폐 관련 이슈 1개를 포함하세요.\n"
        ...
        "theme_stocks는 rising_stocks의 파생 개념입니다. rising_stocks에 포함된 종목이 오를 때,\n"
        "한국 증시 특유의 테마 심리로 인해 역사적으로 함께 급등했던 주변 관련주를 추가로 알려주는 섹션입니다.\n"
        "즉, '이 이슈로 A종목이 오른다 → 과거에 A가 오를 때 B·C도 같이 올랐다'는 패턴을 근거로 선정하세요.\n"
        "- 대장주(1개): rising_stocks 중 이 테마를 가장 강하게 이끄는 핵심 종목 1개. rising_stocks에서 선택 가능.\n"
        "- 직접관련주(1~2개): 대장주의 상승 흐름에 사업 구조상 직접 연동되는 종목. rising_stocks에 없는 종목.\n"
        "- 간접테마주(1~3개): 대장주와 직접 관련은 없지만 과거 동일 이슈 때 시장 심리로 함께 급등한 종목. 역사적 패턴 근거 필수.\n"
        "- ⚠️ 직접관련주·간접테마주는 rising_stocks·falling_stocks에 없는 종목으로만 구성하세요.\n"
        "- 총 3~6개. 실제 반복 패턴이 없으면 억지로 채우지 마세요.\n"
        "- 국내(KOSPI/KOSDAQ) 종목만.\n\n"
