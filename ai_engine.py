...
- 심리·행동: 센티먼트 쏠림, 공포·탐욕, '되는 놈만 되는' 양극화가 개인을 어떻게 함정에 빠뜨리는지.
- 구조적 아이디어·전망: 환율·수급 문제에 대한 구조적 대응(예: 삼성전자·하이닉스의 미국 ADR 상장 가능성 등)이나 향후 분기 시나리오.

[톤·깊이]
- 날카롭고 직설적이되 균형 있게. 구조적 모순과 위험을 명확히 지적하되, 근거 없는 공포 조장이나 단정은 금지 — 반드시 메커니즘·데이터로 뒷받침하세요.
- 충분히 깊고 길게: 7~11개 단락. 각 논점마다 '왜 그런가'의 메커니즘과 구체적 수치·사례·종목을 들어 설명하세요. 표면적 요약 금지.

[리스크·마무리]
- 이 흐름이 되돌려질 때(쏠림 해소, 정책 변화, 금리 방향 전환 등) 무엇이 터질 수 있는지 1~2가지.
- '이런 장에서 개인투자자가 반드시 지켜야 할 냉정한 원칙' 3~4줄. 매도 원칙·현금 비중·욕심 절제 등 구체적 행동 지침으로.

형식: 첫 줄은 제목(마크다운 기호 없이 한 줄, 도발적이되 과하지 않게), 그 다음 줄부터 본문(마크다운 문단, 핵심은 **굵게**, 적당한 이모지)."""
    try:
