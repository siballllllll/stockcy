...
powershell -NoProfile -Command "Get-CimInstance Win32_Process | Where-Object { ($_.Name -eq 'python.exe') -and ($_.CommandLine -like '*multiprocessing.spawn*' -or $_.CommandLine -like '*spawn_main*') } | ForEach-Object { try { Stop-Process -Id $_.ProcessId -Force -ErrorAction Stop; Write-Host ('[OK] killed child PID ' + $_.ProcessId) } catch {} }"

timeout /t 1 /nobreak >nul

echo [*] Step 2: Killing all FastAPI/uvicorn python processes...
powershell -NoProfile -Command "Get-CimInstance Win32_Process | Where-Object { ($_.Name -eq 'python.exe') -and ($_.CommandLine -like '*uvicorn*' -or $_.CommandLine -like '*api.main*' -or $_.CommandLine -like '*run_tunnel*') } | ForEach-Object { try { Stop-Process -Id $_.ProcessId -Force -ErrorAction Stop; Write-Host ('[OK] killed PID ' + $_.ProcessId) } catch {} }"
...
powershell -NoProfile -Command "Get-CimInstance Win32_Process | Where-Object { ($_.Name -eq 'python.exe') -and ($_.CommandLine -like '*run_cloudflared*') } | ForEach-Object { try { Stop-Process -Id $_.ProcessId -Force -ErrorAction Stop; Write-Host ('[OK] killed run_cloudflared PID ' + $_.ProcessId) } catch {} }"
