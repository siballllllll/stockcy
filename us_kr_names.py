# 미국 주식 티커 - 한국어 종목명 매핑 데이터베이스
# 주요 1,000개 이상의 종목 및 ETF 포함

US_KR_NAME_MAP = {
    "AAPL": "애플",
    "MSFT": "마이크로소프트",
    "NVDA": "엔비디아",
    "AMZN": "아마존",
    "GOOGL": "알파벳(구글)",
    "GOOG": "알파벳(구글)",
    "META": "메타",
    "TSLA": "테슬라",
    "BRK-B": "버크셔해서웨이",
    "LLY": "일라이릴리",
    "AVGO": "브로드컴",
    "V": "비자",
    "JPM": "JP모건",
    "TSM": "TSMC",
    "WMT": "월마트",
    "MA": "마스터카드",
    "UNH": "유나이티드헬스",
    "ASML": "ASML",
    "PG": "프록터앤갬블",
    "COST": "코스트코",
    "JNJ": "존슨앤존슨",
    "ORCL": "오라클",
    "HD": "홈디포",
    "MRK": "머크",
    "ABBV": "애브비",
    "CVX": "셰브론",
    "AMD": "AMD",
    "KO": "코카콜라",
    "CRM": "세일즈포스",
    "PEP": "펩시코",
    "ADBE": "어도비",
    "NFLX": "넷플릭스",
    "TMO": "써모피셔",
    "WFC": "웰스파고",
    "BAC": "뱅크오브아메리카",
    "DIS": "디즈니",
    "QCOM": "퀄컴",
    "ACN": "액센츄어",
    "LIN": "린데",
    "INTC": "인텔",
    "VZ": "버라이즌",
    "CSCO": "시스코",
    "ABT": "애보트",
    "INTU": "인튜이트",
    "DHR": "다나허",
    "CAT": "캐터필러",
    "AMAT": "어플라이드머티어리얼즈",
    "TXN": "텍사스인스트루먼트",
    "PFE": "화이자",
    "IBM": "IBM",
    "UBER": "우버",
    "GE": "GE",
    "MS": "모건스탠리",
    "PM": "필립모리스",
    "NEE": "넥스트에라에너지",
    "AMGN": "암젠",
    "GS": "골드만삭스",
    "NOW": "서비스나우",
    "LOW": "로우스",
    "COP": "코노코필립스",
    "RTX": "레이시온",
    "HON": "하니웰",
    "LRCX": "램리서치",
    "PLTR": "팔란티어",
    "BLK": "블랙록",
    "AXP": "아메리칸익스프레스",
    "MU": "마이크론",
    "SCHW": "찰스슈왑",
    "T": "AT&T",
    "MDLZ": "몬델리즈",
    "DE": "디어앤컴퍼니",
    "SYK": "스트라이커",
    "TJX": "TJX",
    "BA": "보잉",
    "VRTX": "버텍스",
    "REGN": "리제네론",
    "SBUX": "스타벅스",
    "BKNG": "부킹홀딩스",
    "ADP": "ADP",
    "LMT": "록히드마틴",
    "BSX": "보스턴사이언티픽",
    "PLD": "프롤로지스",
    "MMC": "마시앤맥레넌",
    "ABNB": "에어비앤비",
    "C": "씨티그룹",
    "ZTS": "조에티스",
    "GILD": "길리어드",
    "CI": "시그나",
    "PANW": "팔로알토",
    "CVS": "CVS헬스",
    "ADI": "아날로그디바이스",
    "FI": "파이서브",
    "KLAC": "KLA",
    "ELV": "엘레반스헬스",
    "CDNS": "케이던스",
    "SNPS": "시놉시스",
    "WM": "웨이스트매니지먼트",
    "EOG": "EOG리소시즈",
    "SHW": "셔윈윌리엄스",
    "MO": "알트리아",
    "ICE": "ICE(인터컨티넨탈)",
    "HCA": "HCA헬스케어",
    "TGT": "타겟",
    "CL": "콜게이트",
    "MCK": "맥케슨",
    "PYPL": "페이팔",
    "NXPI": "NXP반도체",
    "CME": "CME그룹",
    "ORLY": "오라일리",
    "MAR": "메리어트",
    "FDX": "페덱스",
    "APH": "암페놀",
    "Eaton": "이튼",
    "EMR": "에머슨",
    "MCO": "무디스",
    "NOC": "노스롭그루먼",
    "ROP": "로퍼테크놀로지",
    "PH": "파커하니핀",
    "PSX": "필립스66",
    "MDT": "메드트로닉",
    "MRVL": "마벨테크",
    "SNOW": "스노우플레이크",
    "COF": "캐피탈원",
    "AZN": "아스트라제네카",
    "PDD": "핀둬둬(테무)",
    "BABA": "알리바바",
    "TM": "토요타",
    "NVO": "노보노디스크",
    "ASML": "ASML",
    "SAP": "SAP",
    "SONY": "소니",
    "SHOP": "쇼피파이",
    "SPOT": "스포티파이",
    "BIDU": "바이두",
    "LI": "리 오토",
    "NIO": "니오",
    "XPEV": "샤오펑",
    "TSM": "TSMC",
    "ARM": "ARM",
    "SMCI": "슈퍼마이크로",
    "AVGO": "브로드컴",
    "APP": "앱러빈",
    "U": "유니티",
    "SQ": "블록(스퀘어)",
    "AFRM": "어펌",
    "SOFI": "소파이",
    "NU": "누뱅크",
    "HOOD": "로빈후드",
    "COIN": "코인베이스",
    "MSTR": "마이크로스트래티지",
    "MBLY": "모빌아이",
    "LAZR": "루미나",
    "RIVN": "리비안",
    "LCID": "루시드",
    "DKNG": "드래프트킹스",
    "ROKU": "로쿠",
    "ZM": "줌",
    "DOCU": "도큐사인",
    "TEAM": "아틀라시안",
    "DDOG": "다타독",
    "MDB": "몽고DB",
    "OKTA": "옥타",
    "CRWD": "크라우드스트라이크",
    "ZS": "지스케일러",
    "NET": "클라우드플레어",
    "PATH": "유아이패스",
    "UNITY": "유니티",
    "SE": "씨(쇼피)",
    "MELI": "메르카도리브레",
    "CPNG": "쿠팡",
    "DASH": "도어대시",
    "ABNB": "에어비앤비",
    "LYFT": "리프트",
    "AAL": "아메리칸항공",
    "DAL": "델타항공",
    "UAL": "유나이티드항공",
    "LUV": "사우스웨스트항공",
    "CCL": "카니발",
    "RCL": "로열캐리비안",
    "NCLH": "노르웨이지안",
    "BKNG": "부킹홀딩스",
    "EXPE": "익스피디아",
    "TRIP": "트립어드바이저",
    "MAR": "메리어트",
    "HLT": "힐튼",
    "WYNN": "윈리조트",
    "LVS": "라스베이거스샌즈",
    "MGM": "MGM리조트",
    "AMC": "AMC엔터",
    "GME": "게임스탑",
    "KOSS": "코스",
    "BB": "블랙베리",
    "NOK": "노키아",
    "AI": "C3.ai",
    "SOUN": "사운드하운드",
    "BROS": "더치브로스",
    "CMG": "치폴레",
    "SHAK": "쉐이크쉑",
    "WING": "윙스탑",
    "DPZ": "도미노피자",
    "YUM": "얌브랜즈",
    "MCD": "맥도날드",
    "SBUX": "스타벅스",
    "NKE": "나이키",
    "LULU": "룰루레몬",
    "UA": "언더아머",
    "SKX": "스케쳐스",
    "CROX": "크록스",
    "DECK": "데커스(호카)",
    "ONON": "온홀딩",
    "TSLA": "테슬라",
    "F": "포드",
    "GM": "GM",
    "STLA": "스텔란티스",
    "HMC": "혼다",
    "TM": "토요타",
    "VALE": "발레",
    "RIO": "리오틴토",
    "BHP": "BHP",
    "FCX": "프리포트맥모란",
    "ALB": "알베말",
    "SQM": "SQM",
    "LAC": "리튬아메리카스",
    "MP": "MP머티어리얼스",
    "X": "US스틸",
    "CLF": "클리프스",
    "NUE": "뉴코어",
    "STLD": "스틸다이나믹스",
    "AA": "알코아",
    "PFE": "화이자",
    "MRNA": "모더나",
    "BNTX": "바이오엔텍",
    "NVAX": "노바백스",
    "JNJ": "존슨앤존슨",
    "LLY": "일라이릴리",
    "NVO": "노보노디스크",
    "MRK": "머크",
    "ABBV": "애브비",
    "BMY": "브리스톨마이어스",
    "GILD": "길리어드",
    "AMGN": "암젠",
    "REGN": "리제네론",
    "VRTX": "버텍스",
    "BIIB": "바이오젠",
    "IONQ": "아이온큐",
    "RGTI": "리게티",
    "QUBT": "퀀텀컴퓨팅",
    "PLTR": "팔란티어",
    "OKLO": "오클로",
    "SMR": "뉴스케일",
    "CCJ": "카메코",
    "VST": "비스트라",
    "CEG": "컨스텔레이션",
    "TLT": "20년물국채ETF",
    "TMF": "20년물국채3배ETF",
    "TLW": "20년물국채1배ETF",
    "SHY": "1-3년물국채ETF",
    "IEF": "7-10년물국채ETF",
    "SPY": "S&P500 ETF",
    "VOO": "S&P500 ETF(뱅가드)",
    "IVV": "S&P500 ETF(iShares)",
    "QQQ": "나스닥100 ETF",
    "TQQQ": "나스닥100 3배 ETF",
    "SQQQ": "나스닥100 인버스 3배 ETF",
    "QLD": "나스닥100 2배 ETF",
    "PSQ": "나스닥100 인버스 ETF",
    "SOXX": "반도체 ETF",
    "SOXL": "반도체 3배 ETF",
    "SOXS": "반도체 인버스 3배 ETF",
    "DIA": "다우존스 ETF",
    "IWM": "러셀2000 ETF",
    "ARKK": "아크이노베이션 ETF",
    "ARKW": "아크차세대인터넷 ETF",
    "ARKG": "아크게노믹 ETF",
    "ARKF": "아크핀테크 ETF",
    "SCHD": "배당성장 ETF(SCHD)",
    "JEPI": "커버드콜 ETF(JEPI)",
    "JEPQ": "나스닥 커버드콜 ETF(JEPQ)",
    "QYLD": "나스닥 커버드콜 ETF(QYLD)",
    "XYLD": "S&P500 커버드콜 ETF(XYLD)",
    "GLD": "금 ETF",
    "IAU": "금 ETF(iShares)",
    "SLV": "은 ETF",
    "USO": "원유 ETF",
    "UNG": "천연가스 ETF",
    "BITO": "비트코인선물 ETF",
    "IBIT": "비트코인현물 ETF(블랙록)",
    "FBTC": "비트코인현물 ETF(피델리티)",
    "ARKB": "비트코인현물 ETF(아크)",
    "TSLL": "테슬라 1.5/2배 ETF",
    "NVDL": "엔비디아 2배 ETF",
    "XRX": "제록스",
    "IBM": "IBM",
    "DELL": "델테크놀로지스",
    "HPQ": "HP",
    "HPE": "HP엔터프라이즈",
    "WDC": "웨스턴디지털",
    "STX": "씨게이트",
    "PSTG": "퓨어스토리지",
    "NTAP": "넷앱",
    "ANET": "아리스타",
    "GLW": "코닝",
    "MPWR": "모노리식파워",
    "LSCC": "래티스반도체",
    "CRUS": "시러스로직",
    "SNPS": "시놉시스",
    "CDNS": "케이던스",
    "GFS": "글로벌파운드리",
    "UMC": "UMC",
    "AMAT": "어플라이드머티어리얼즈",
    "LRCX": "램리서치",
    "KLAC": "KLA",
    "ASML": "ASML",
    "TER": "테라다인",
    "COHU": "코후",
    "ENTG": "엔테그리스",
    "MU": "마이크론",
    "WDC": "웨스턴디지털",
    "STX": "씨게이트",
    "ANET": "아리스타",
    "GLW": "코닝",
    "MPWR": "모노리식파워",
    "LSCC": "래티스반도체",
    "CRUS": "시러스로직",
    "PLTR": "팔란티어",
    "AI": "C3.ai",
    "SOUN": "사운드하운드",
    "APLD": "어플라이드디지털",
    "CRM": "세일즈포스",
    "NOW": "서비스나우",
    "SNOW": "스노우플레이크",
    "WDAY": "워크데이",
    "ADBE": "어도비",
    "PATH": "유아이패스",
    "DDOG": "다타독",
    "MDB": "몽고DB",
    "HUBS": "허브스팟",
    "ZM": "줌",
    "ESTC": "엘라스틱",
    "DT": "다이나트레이스",
    "SNAP": "스냅",
    "PINS": "핀터레스트",
    "RDDT": "레딧",
    "MTCH": "매치그룹",
    "BMBL": "범블",
    "TTD": "트레이드데스크",
    "APP": "앱러빈",
    "IPG": "IPG",
    "OMC": "옴니콤",
    "DV": "더블버파이",
    "LLY": "일라이릴리",
    "NVO": "노보노디스크",
    "PFE": "화이자",
    "MRK": "머크",
    "ABBV": "애브비",
    "BMY": "브리스톨마이어스",
    "AZN": "아스트라제네카",
    "JNJ": "존슨앤존슨",
    "MRNA": "모더나",
    "BNTX": "바이오엔텍",
    "NVAX": "노바백스",
    "GILD": "길리어드",
    "AMGN": "암젠",
    "VRTX": "버텍스",
    "REGN": "리제네론",
    "BIIB": "바이오젠",
    "RXRX": "리커루션",
    "ISRG": "인튜이티브서지컬",
    "ABT": "애보트",
    "MDT": "메드트로닉",
    "DXCM": "덱스콤",
    "SYK": "스트라이커",
    "EW": "에드워즈라이프",
    "HOLX": "홀로직",
    "UNH": "유나이티드헬스",
    "CVS": "CVS헬스",
    "CI": "시그나",
    "HUM": "험비",
    "ELV": "엘레반스",
    "LMT": "록히드마틴",
    "RTX": "레이시온",
    "NOC": "노스롭그루먼",
    "GD": "제너럴다이내믹스",
    "LHX": "L3해리스",
    "TDG": "트랜스다임",
    "HEI": "HEICO",
    "RKLB": "로켓랩",
    "LUNR": "인튜이티브머신스",
    "ASTS": "AST스페이스모바일",
    "SPIR": "스파이어글로벌",
    "AVAV": "에어로바이런먼트",
    "TXT": "텍스트론",
    "KTOS": "크라토스",
    "AXON": "액슨엔터프라이즈",
    "CACI": "CACI",
    "BAH": "부즈앨런",
    "LDOS": "레이도스",
    "SAIC": "SAIC",
    "CRWD": "크라우드스트라이크",
    "S": "센티넬원",
    "PANW": "팔로알토",
    "RPD": "래피드7",
    "VRNS": "바로니스",
    "OTEX": "오픈텍스트",
    "ZS": "지스케일러",
    "FTNT": "포티넷",
    "OKTA": "옥타",
    "CYBR": "사이버아크",
    "NET": "클라우드플레어",
    "CHKP": "체크포인트",
    "TENB": "테너블",
    "QLYS": "퀄리스",
    "FFIV": "F5네트웍스",
    "RDWR": "래드웨어",
    "FROG": "제이프로그",
    "GTLB": "깃랩",
    "RBRK": "루브릭",
    "IONQ": "아이온큐",
    "QBTS": "디웨이브",
    "RGTI": "리게티",
    "QUBT": "퀀텀컴퓨팅",
    "ARQQ": "아퀴트",
    "COHR": "코히런트",
    "LITE": "루멘텀",
    "MKSI": "MKS인스트루먼트",
    "CCJ": "카메코",
    "CEG": "컨스텔레이션에너지",
    "VST": "비스트라",
    "OKLO": "오클로",
    "LNG": "셰니어에너지",
    "EQT": "EQT",
    "CTRA": "코테라에너지",
    "RRC": "레인지리소시즈",
    "ENPH": "인페이즈에너지",
    "FSLR": "퍼스트솔라",
    "NEE": "넥스트에라에너지",
    "RUN": "선런",
    "SEDG": "솔라엣지",
    "XOM": "엑슨모빌",
    "CVX": "셰브론",
    "OXY": "옥시덴탈",
    "COP": "코노코필립스",
    "GEV": "GE베르노바",
    "VRT": "버티브",
    "ETN": "이튼",
    "PWR": "퀀타서비스",
    "EME": "엠코",
    "MYRG": "MYR그룹",
    "AEP": "아메리칸일렉트릭파워",
    "EXC": "엑셀론",
    "ETR": "엔터지",
    "PEG": "PSEG",
    "D": "도미니온",
    "PCG": "PG&E",
    "FLNC": "플루언스에너지",
    "QS": "퀀텀스케이프",
    "FLUX": "플럭스파워",
    "JPM": "JP모건",
    "BAC": "뱅크오브아메리카",
    "GS": "골드만삭스",
    "MS": "모건스탠리",
    "WFC": "웰스파고",
    "C": "씨티그룹",
    "V": "비자",
    "MA": "마스터카드",
    "PYPL": "페이팔",
    "SQ": "블록",
    "AFRM": "어펌",
    "SOFI": "소파이",
    "NU": "누홀딩스",
    "COIN": "코인베이스",
    "MSTR": "마이크로스트래티지",
    "HOOD": "로빈후드",
    "BRK-B": "버크셔해서웨이",
    "BLK": "블랙록",
    "SCHW": "찰스슈왑",
    "AXP": "아메리칸익스프레스",
    "FIS": "FIS",
    "TSLA": "테슬라",
    "RIVN": "리비안",
    "LCID": "루시드",
    "NIO": "니오",
    "ON": "온세미",
    "CHPT": "차지포인트",
    "BLNK": "블링크차징",
    "BWA": "보그워너",
    "ALB": "알베말",
    "ABB": "ABB",
    "HON": "하니웰",
    "IRBT": "이로봇",
    "MBLY": "모빌아이",
    "LAZR": "루미나",
    "UBER": "우버",
    "LYFT": "리프트",
    "AUR": "오로라",
    "AMZN": "아마존",
    "SHOP": "쇼피파이",
    "EBAY": "이베이",
    "ETSY": "엣시",
    "PDD": "핀둬둬",
    "WMT": "월마트",
    "COST": "코스트코",
    "TGT": "타겟",
    "HD": "홈디포",
    "LOW": "로우스",
    "DG": "달러제너럴",
    "NKE": "나이키",
    "UA": "언더아머",
    "LULU": "룰루레몬",
    "PVH": "PVH",
    "ONON": "온홀딩",
    "PG": "프록터앤갬블",
    "KO": "코카콜라",
    "PEP": "펩시코",
    "CL": "콜게이트",
    "UL": "유니레버",
    "MCD": "맥도날드",
    "SBUX": "스타벅스",
    "CMG": "치폴레",
    "YUM": "얌브랜즈",
    "BROS": "더치브로스",
    "NFLX": "넷플릭스",
    "DIS": "디즈니",
    "WBD": "워너브라더스",
    "PARA": "파라마운트",
    "SPOT": "스포티파이",
    "LYV": "라이브네이션",
    "RBLX": "로블록스",
    "EA": "EA",
    "TTWO": "테이크투",
    "U": "유니티",
    "TMUS": "T모바일",
    "VZ": "버라이즌",
    "T": "AT&T",
    "VSAT": "비아샛",
    "GSAT": "글로벌스타",
    "CMCSA": "컴캐스트",
    "CHTR": "차터",
    "CSCO": "시스코",
    "DAL": "델타항공",
    "UAL": "유나이티드항공",
    "AAL": "아메리칸항공",
    "LUV": "사우스웨스트항공",
    "BKNG": "부킹홀딩스",
    "ABNB": "에어비앤비",
    "EXPE": "익스피디아",
    "TRIP": "트립어드바이저",
    "CCL": "카니발",
    "RCL": "로열캐리비안",
    "HLT": "힐튼",
    "MAR": "메리어트",
    "EQIX": "에퀴닉스",
    "DLR": "디지털리얼티",
    "IRM": "아이언마운틴",
    "PLD": "프롤로지스",
    "AMT": "아메리칸타워",
    "CCI": "크라운캐슬",
    "SBAC": "SBA커뮤니케이션즈",
    "SPG": "사이먼프로퍼티",
    "O": "리얼티인컴",
    "AVB": "아발론베이",
    "EQR": "에퀴티레지덴셜",
    "NUE": "뉴코어",
    "X": "US스틸",
    "FCX": "프리포트맥모란",
    "CLF": "클리프스",
    "DOW": "다우",
    "DD": "듀폰",
    "APD": "에어프로덕츠",
    "LIN": "린데",
    "CAT": "캐터필러",
    "DE": "디어앤컴퍼니",
    "VMC": "불캔머티어리얼스",
    "FLR": "플루오르",
    "F": "포드",
    "GM": "GM",
    "STLA": "스텔란티스",
    "NEM": "뉴몬트",
    "GOLD": "배릭골드",
    "AEM": "아그니코이글",
    "WPM": "휠튼프레셔스",
    "AU": "앵글로골드",
    "SCCO": "서던코퍼",
    "BHP": "BHP",
    "RIO": "리오틴토",
    "SQM": "SQM",
    "MP": "MP머티어리얼스",
    "LAC": "리튬아메리카스",
    "SOXX": "반도체 ETF",
    "SOXL": "반도체 3배 ETF",
    "SOXS": "반도체 인버스 3배 ETF",
    "SMH": "반도체 ETF(Vaneck)",
    "QQQ": "나스닥100 ETF",
    "TQQQ": "나스닥100 3배 ETF",
    "SQQQ": "나스닥100 인버스 3배 ETF",
    "SPY": "S&P500 ETF",
    "VOO": "S&P500 ETF(뱅가드)",
    "IVV": "S&P500 ETF(iShares)",
    "DIA": "다우존스 ETF",
    "IWM": "러셀2000 ETF",
    "TLT": "20년물국채 ETF",
    "TMF": "20년물국채 3배 ETF",
    "IEF": "7-10년물국채 ETF",
    "SHY": "1-3년물국채 ETF",
    "GLD": "금 ETF",
    "IAU": "금 ETF(iShares)",
    "SLV": "은 ETF",
    "USO": "원유 ETF",
    "UNG": "천연가스 ETF",
    "BITO": "비트코인선물 ETF",
    "IBIT": "비트코인현물 ETF",
    "SCHD": "배당성장 ETF(SCHD)",
    "JEPI": "커버드콜 ETF(JEPI)",
    "JEPQ": "나스닥 커버드콜 ETF(JEPQ)",
    "ONDS": "온다스",
    "LUNR": "인튜이티브 머신스 (NASA 민간 우주선 개발, LUNR)",
    "ASTS": "AST 스페이스모바일 (저궤도 위성 통신, ASTS)",
    "RKLB": "로켓 랩 (우주 발사 및 소형위성 제작, RKLB)",
    "PLTR": "팔란티어 테크놀로지스 (PLTR)",
    "SNOW": "스노우플레이크",
    "U": "유니티",
    "SQ": "블록(스퀘어)",
    "COIN": "코인베이스 글로벌 (미국 최대 가상자산거래소, COIN)",
    "MSTR": "마이크로스트래티지 (비트코인 최대보유사, MSTR)",
    "HOOD": "로빈후드",
    "AFRM": "어펌",
    "SOFI": "소파이 테크놀로지스 (SOFI)",
    "UPST": "업스타트 홀딩스 (AI 신용대출 플랫폼, UPST)",
    "MARA": "마라홀딩스 (비트코인 채굴기업, MARA)",
    "RIOT": "라이엇 플랫폼즈 (비트코인 채굴, RIOT)",
    "CLSK": "클린스파크 (친환경 비트코인 채굴, CLSK)",
    "IREN": "아이리스 에너지 (AI 데이터센터 및 채굴, IREN)",
    "WULF": "테라울프 (친환경 채굴, WULF)",
    "CIFR": "사이퍼 마이닝",
    "BTBT": "비트 디지털",
    "BITF": "비트팜스",
    "HIVE": "하이브 디지털",
    "CAN": "가나안",
    "EH": "이항",
    "Joby": "조비 에비에이션",
    "ACHR": "아처 에비에이션",
    "VRT": "버티브 홀딩스",
    "GCT": "기가클라우드",
    "APP": "앱러빈",
    "TQQQ": "프로셰어즈 나스닥100 3배 레버리지 (TQQQ)",
    "SQQQ": "프로셰어즈 나스닥100 인버스 3배 (SQQQ)",
    "SOXL": "디렉시온 필라델피아반도체 3배 레버리지 (SOXL)",
    "SOXS": "디렉시온 필라델피아반도체 인버스 3배 (SOXS)",
    "TSLL": "디렉시온 테슬라 2배 레버리지 (TSLL)",
    "NVDL": "그래니트shares 2배 엔비디아 레버리지 (NVDL)",
}

_WORD_MAP = {
    "Technology": "테크놀로지", "Technologies": "테크놀로지스", "Systems": "시스템즈",
    "Digital": "디지털", "Energy": "에너지", "Global": "글로벌", "International": "인터내셔널",
    "Networks": "네트웍스", "Medical": "메디컬", "Health": "헬스", "Solutions": "솔루션즈",
    "Group": "그룹", "Holdings": "홀딩스", "Capital": "캐피탈", "Financial": "파이낸셜",
    "Electric": "일렉트릭", "Airlines": "항공", "Mining": "마이닝", "Materials": "머티어리얼즈",
    "Resources": "리소시즈", "Trust": "트러스트", "Fund": "펀드", "Growth": "그로스",
    "Value": "밸류", "Dividend": "배당", "High Yield": "고배당", "ETF": "ETF",
    "Semiconductor": "반도체", "Auto": "오토", "Software": "소프트웨어",
    "Cloud": "클라우드", "Solar": "솔라", "Bio": "바이오", "Pharma": "파마",
    "Therapeutics": "테라퓨틱스", "Communications": "커뮤니케이션즈",
    "Power": "파워", "Petroleum": "석유", "Gas": "가스", "Oil": "오일",
    "Bank": "은행", "Insurance": "보험", "Investment": "투자", "Real Estate": "부동산",
}

def get_kr_name(ticker, english_name):
    """티커에 해당하는 한국어 이름을 반환하거나, 영어 이름을 가공하여 반환."""
    # 1. 수동 매핑 리스트 확인
    if ticker in US_KR_NAME_MAP:
        return US_KR_NAME_MAP[ticker]
    
    # 2. 영어 이름 가공 (Suffix 제거)
    import re
    cleaned = re.sub(r'\s+(Inc|Corp|Ltd|S\.A\.|plc|Group|Holdings|Co|Company|Incorporated|Corporation|Limited|Plc|L\.P\.)(\.|\b).*', '', english_name, flags=re.I)
    cleaned = cleaned.strip()
    
    # 3. 지능형 단어 변환 (Technology -> 테크놀로지 등)
    words = cleaned.split()
    translated_words = []
    for w in words:
        # 문장 부호 제거 후 비교
        base_w = re.sub(r'[^a-zA-Z]', '', w)
        if base_w in _WORD_MAP:
            translated_words.append(_WORD_MAP[base_w])
        elif base_w.capitalize() in _WORD_MAP:
            translated_words.append(_WORD_MAP[base_w.capitalize()])
        else:
            translated_words.append(w)
    
    cleaned = " ".join(translated_words)
    
    # 너무 길면 자름
    if len(cleaned) > 30:
        cleaned = cleaned[:29] + ".."
        
    return cleaned
